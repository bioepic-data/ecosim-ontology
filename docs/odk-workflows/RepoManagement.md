...
1. Add an import statement to your `src/ontology/bervo-edit.owl` file. We suggest to do this using a text editor, by simply copying an existing import declaration and renaming it to the new ontology import, for example as follows:
    ```
    ...
...
1. remove the import declaration from your `src/ontology/bervo-edit.owl`.
2. remove the id from your `src/ontology/bervo-odk.yaml`, eg. `- id: go` from the list of `products` in the `import_group`.
...
Now add a new goal in your custom Makefile (`src/ontology/bervo.Makefile`, _not_ `src/ontology/Makefile`).
...
1. Open `src/ontology/bervo-odk.yaml`
...
1. Providing a `components/mycomp.owl:` make target in `src/ontology/bervo.Makefile`
...
1. Providing an additional attribute for the component in `src/ontology/bervo-odk.yaml`, `source`,
